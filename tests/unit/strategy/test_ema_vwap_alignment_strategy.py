...
from tests.unit.strategy.fixtures import (
    ...
    BULLISH_CLOSES,
    CONFLICTING_DOWNTREND_CLOSES,
    CONFLICTING_UPTREND_CLOSES,
    INSUFFICIENT_CLOSES,
)
...
def test_bullish_setup_detected() -> None:
    strategy = EMAVWAPAlignmentStrategy()
    series, as_of = _candles_for(BULLISH_CLOSES)
...
def test_bearish_setup_detected() -> None:
    strategy = EMAVWAPAlignmentStrategy()
    series, as_of = _candles_for(BEARISH_CLOSES)
    market_state = _market_state(as_of)
...
@pytest.mark.parametrize("closes", [CONFLICTING_UPTREND_CLOSES, CONFLICTING_DOWNTREND_CLOSES])
